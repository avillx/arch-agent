...
type A2AInterceptor struct {
	...
	callCh  <-chan service.A2ACall
	respCh  <-chan service.A2AResponse

	requestMessageQueue map[service.A2ACall]tgbotapi.Message
	mu                  sync.RWMutex
}

func NewA2AInterceptor(groupID int64, orch *BotOrchestrator, a2a *service.A2AService) *A2AInterceptor {
	return &A2AInterceptor{
		groupID: groupID,
		orch:    orch,
		callCh:  a2a.CallChannel(),
		respCh:  a2a.ResponseChannel(),

		requestMessageQueue: make(map[service.A2ACall]tgbotapi.Message),
	}
}
...
func (i *A2AInterceptor) onA2ACall(_ context.Context, c service.A2ACall) error {
...
func (i *A2AInterceptor) onA2AResponse(_ context.Context, r service.A2AResponse) error {
	...
	i.mu.RLock()
	msg, ok := i.requestMessageQueue[r.A2ACall]
	i.mu.RUnlock()
	...
	i.mu.Lock()
	defer i.mu.Unlock()
	delete(i.requestMessageQueue, r.A2ACall)
